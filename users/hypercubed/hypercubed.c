...
void register_asciimap(uint16_t x) {
    const char* code = ascii_map[x];
    ascii_input_start();
    for (int i = 0; i < strlen(code); ++i) {
        tap_code(numpad_key_map[(int)(code[i]-'0')]);
    }
    ascii_input_finish();
}
...
bool send(uint16_t x, uint16_t y, keyrecord_t *record) {
    if (record->event.pressed) {
        uint8_t idx = get_index(x, y);

        #ifdef CONSOLE_ENABLE
            uprintf("send_unicode_up: kc: 0x%04X\n", idx);
        #endif

        if (get_unicode_input_mode() == UNICODE_MODE_WINDOWS) {
            if (!process_ascii_overrides(idx)) {
                return false;
            }
        }
        register_unicodemap(idx);
    }
    return false;
}
